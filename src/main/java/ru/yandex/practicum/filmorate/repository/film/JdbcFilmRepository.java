...
public class JdbcFilmRepository extends JdbcBaseRepository<Film> implements FilmRepository {
    private static final String GET_ALL_QUERY = "SELECT films.*, mpa.name AS mpa_name, genres.genre_id, genres.name AS genre_name FROM films " +
    ...
    private static final String GET_FILM_BY_ID = "SELECT films.*, mpa.name AS mpa_name, genres.genre_id, genres.name AS genre_name FROM films " +
            "LEFT OUTER JOIN mpa ON mpa.mpa_id = films.mpa_id " +
            "LEFT OUTER JOIN films_genres ON films_genres.film_id = films.film_id " +
            "LEFT OUTER JOIN genres ON genres.genre_id = films_genres.genre_id " +
    ...
    private static final String INSERT_FILMS_GENRES_BY_FILM_ID_QUERY = "INSERT INTO films_genres(film_id, genre_id) " +
    ...
    private static final String ADD_LIKE_QUERY = "MERGE INTO likes AS t " +
    ...
    private static final String GET_MOST_POPULAR_QUERY = "SELECT FILMS.*, MPA.NAME AS MPA_NAME, GENRES.GENRE_ID, GENRES.NAME AS GENRE_NAME " +
            "FROM FILMS " +
            "LEFT OUTER JOIN MPA ON MPA.MPA_ID = FILMS.MPA_ID " +
            "LEFT OUTER JOIN FILMS_GENRES ON FILMS_GENRES.FILM_ID = FILMS.FILM_ID " +
            "LEFT OUTER JOIN GENRES ON GENRES.GENRE_ID = FILMS_GENRES.GENRE_ID " +
